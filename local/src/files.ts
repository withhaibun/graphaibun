...
function getFileType(filePath: string): string {
    ...
    if (fs.statSync(filePath).isFile()) return 'file';
    if (fs.statSync(filePath).isSymbolicLink()) return 'symlink';
    return 'unknown';
}
...
const ext = (file: string) => file.replace(/.*\./, '');

export function goToThere(dirPath: string, parentId: number, depth: number) {
    ...
    files.forEach(file => {
        ...
        if (fileType === 'directory' && file !== 'node_modules') {
            try {
                ...
                console.error(filePath, 'failed:', (e as any).message);
            }
        }
    });
}
